...
logging.basicConfig(filename='logs/mqtt_pub.csv', format='%(asctime)s.%(msecs)03d,%(message)s',
...
def mqtt_pub():
    ...
    print("Starting as MQTT Publisher...")
    ...
    rpi_client = mqtt.Client(client_id='mqtt_pub', protocol=mqtt.MQTTv5)
    rpi_client.kill_flag = False
    rpi_client.reconnect_flag = True
    rpi_client.enable_logger(logger=logging.Logger('mqtt_pub'))
    ...
    while rpi_client.is_connected() and not rpi_client.kill_flag:
        ...
        # publish here
        if rpi_client.change_flag or True:
            rpi_client.publish(topic=IMU_SENSOR_TOPIC, payload='<PAYLOAD HERE>', qos=2, retain=True)
        time.sleep(REFRESH_RATE)
